...
template <int dim>
MG2HeatSource<dim>::MG2HeatSource(boost::property_tree::ptree const &database)
    : HeatSource<dim>(database)
{
}

template <int dim>
void MG2HeatSource<dim>::update_time(double time)
{
  ...
  _alpha =
      ...
      (this->_beam.radius_squared * this->_beam.depth * _pi_to_1p5_times_0p5);
}

template <int dim>
double MG2HeatSource<dim>::value(dealii::Point<dim> const &point,
                                 double const height) const
{
  ...
  else
  {
    ...
    double heat_source =
        _alpha * std::exp(-xpy_squared / this->_beam.radius_squared -
                          std::pow(z / this->_beam.depth, 2));
